...
Go runtime 刷新 AI Day 时会把 ATM 已有会话镜像归一化为 `ai_day_events`。事件 ID 幂等，包含时间、来源、哈希
...
来源开关、语义分类、保留期和删除由 Web 的数据与隐私页管理。暂停不会删除历史；来源删除会删除
...
Web 的 AI Day 快照走 `aiday.Service` 的 `Refresh`：补齐基线窗口内**从未构建过**的日子，并且总是
重建当天，已经构建过的过去保持不变。此前每次读取都会重建整个 31 天窗口，意味着打开一次页面就可能
...
诊断，`export` 用于数据携带和支持取证。今日、历史、Atlas、反馈、隐私与删除都是 Web 能力，直接调用
同一个 `aiday.Service`，不再占用 Agent 的命令命名空间。

Web 的 AI Day 工作区包含今日、Atlas 星图/列表、历史、数据与隐私四个页面。今日卡显示
