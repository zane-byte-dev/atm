...
ATM 已于 2026-08-04 在 `zane-byte-dev/atm` 公开发布，当前适合早期用户试用。核心功能已经闭环：
...
公开仓库使用单一根提交起步，原私有历史保留在独立私有仓库中。首轮 CI、发布契约检查和 CodeQL 均已通过。

## 开源发布门禁

公开发布检查结果：

- [x] 仓库所有者已确认执行公开发布；权利归属和适用授权由发布者负责确认及留存。
...
- [x] 在 GitHub 启用分支保护、私密漏洞报告、Dependabot 安全更新和 secret scanning。

权利归属、内部能力边界和历史隐私仍需由维护者持续复核，不能用测试通过替代。
