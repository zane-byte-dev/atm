...
- **Todo refine 是一次 schema 调用，不是 Agent 循环**（2026-08-13）：人刚写下的任务常常是一句
  话。`todo refine` 用和收集相同的隔离 CLI 链润色标题/需求，复杂时写计划并拆出子任务。它不
  派发执行、不改优先级、不发明项目。命令行 `todo add` 保持即时，只有 `--refine` 才跟着整理；
  桌面添加默认自动跑，因为那是人随手记下的入口。`in_progress` 只润色不拆分，避免把正在工作的
  会话解绑。
