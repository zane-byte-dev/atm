...
点击状态项直接打开普通 macOS 菜单。状态栏沿用 Web 的圆角 A 标识，默认只追加紧凑的今日 Token；不常驻显示任务或会话数量，有待处理事项或额度达到 75% 警戒线时再扩展成例如 `待 2 · Codex 92% · 387M`。菜单顶部显示 Go 服务状态和只读的“今日 Token”，随后展示当前任务与最多 3 条 Agent 额度摘要；只有内容被折叠时才显示“查看全部”入口。任务行把状态和项目收进悬浮提示，底部提供打开 ATM、新增任务、同步并刷新、设置和退出。任务与额度行通过一次性 ticket 打开对应 Web 页面。全局快捷键默认是 ⌥⌘A，用于直接打开 ATM Web 工作区。

App 每 10 秒调用 `POST /api/v1/control/companion`，读取 `snapshot`、通知 `feed`、`todos`、`quota` 和 `today_usage`。`today_usage` 的 `total_tokens` 为零时明确显示 0，读取失败时显示“暂不可用”。菜单中的“同步并刷新”只要求 Go runtime 排队一次同步，实际同步仍由 Go 后台完成。
