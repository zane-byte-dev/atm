...
enum ATMVoiceInputState: Equatable {
    case idle
    ...
    var isActive: Bool {
        switch self {
        case .requestingPermission, .recording, .processing: return true
...
@MainActor
final class ATMVoiceInputCoordinator: ObservableObject {
    static let shared = ATMVoiceInputCoordinator()

    @Published private(set) var state: ATMVoiceInputState = .idle
    /// Kept after the fact so settings can offer it for copying — the only way back to
    /// a transcript whose paste failed.
    @Published private(set) var lastTranscript = ""
    @Published private(set) var lastOutcome: ATMTextInjector.Outcome?
    @Published private(set) var activeEngineName = ""
    @Published private(set) var showsLevelMeter = false
    @Published private(set) var inputLevel: Float = 0

    private let transcriber: ATMVoiceTranscriberRouter
    ...
    /// The modifiers the recording started with, for the watchdog to compare against.
    private var heldModifiers: NSEvent.ModifierFlags = []

    init(transcriber: ATMVoiceTranscriberRouter? = nil) {
        self.transcriber = transcriber ?? ATMVoiceTranscriberRouter()
    }
    ...
    func hotKeyPressed() {
        ...
        guard ATMAppBundle.isBundled else {
            ATMLog.failure(
                "voice_input_requires_app_bundle",
                fields: ["bundle": Bundle.main.bundleURL.lastPathComponent]
            )
            generation += 1
            transition(to: .failed(ATMSpeechTranscriberError.requiresAppBundle.localizedDescription))
    ...
    private func beginRecording() {
        ...
        heldModifiers = ATMVoiceInputPreferences.hotKey.modifiers
        ATMGlobalHotKeyManager.shared.registerTransient(
            .cancelVoice,
            hotKey: ATMVoiceInputPreferences.cancelHotKey
        )
        ...
        operationTask = Task { [weak self] in
            ...
            do {
                ...
                startLevelUpdates(generation: currentGeneration)
                ...
                Task { [weak self] in await self?.transcriber.prepare() }
    ...
    private func finishRecordingSoon() {
        ...
        operationTask?.cancel()
        operationTask = Task { [weak self] in
            await self?.finishRecording(generation: currentGeneration)
        }
    }

    private func finishRecording(generation currentGeneration: Int) async {
        ...
        let transcript = ATMVoiceTextCleanup.process(raw)
        ...
        do {
            let outcome = try await ATMTextInjector.inject(transcript, into: targetApplication, isCurrent: {
    ...
    ///
    /// Carbon reports hot key release for the registration as a whole, and there are
    /// keyboard states — releasing ⌥ before Space, a Space switch mid-hold — where that
    /// report does not arrive. `NSEvent.modifierFlags` is a plain static read requiring
    /// no permission, so polling it answers "is the key still held" without watching the
    /// keyboard. The duration cap is the backstop for the case where even that lies.
    private func startWatchdog(generation currentGeneration: Int) {
        stopWatchdog()
        let deadline = ATMVoiceInputPreferences.maximumRecordingDuration
        watchdogTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                let stillHeld = NSEvent.modifierFlags
                    .intersection(ATMHotKey.supportedModifiers)
                    .isSuperset(of: self.heldModifiers)
                if !stillHeld {
                    self.hotKeyReleased()
                    return
                }
    ...
    /// rates.
    private func startLevelUpdates(generation currentGeneration: Int) {
        stopLevelUpdates()
        levelTask = Task { [weak self] in
            while !Task.isCancelled {
                guard let self, currentGeneration == self.generation, self.state.isRecording else { return }
                self.inputLevel = self.transcriber.inputLevel
    ...
    private func fail(_ message: String, generation currentGeneration: Int) {
        ...
        transcriber.cancel()
        teardown()
        ATMLog.failure("voice_input_failed", error: message)
        transition(to: .failed(message))
        // Long enough to actually read a permission instruction.
    ...
    private func teardown() {
        stopLevelUpdates()
        stopWatchdog()
        ATMGlobalHotKeyManager.shared.unregisterTransient(.cancelVoice)
        releaseRequested = false
        heldModifiers = []
    }
    ...
    private func transition(to newState: ATMVoiceInputState) {
        state = newState
        if case .idle = newState {
            ATMVoiceOverlayController.shared.hide()
        } else {
            ATMVoiceOverlayController.shared.show(coordinator: self)
        }
    }
}
