...
/// Four-char code identifying VoxCaret's hot keys inside the process-wide Carbon hot
/// key table, so the shared event handler ignores registrations made by anyone
/// else linked into this process.
private let atmHotKeySignature = OSType(0x41_54_4D_4B) // 'VoxCaretK'

/// Which of VoxCaret's global shortcuts a Carbon registration belongs to.
///
...
enum VoxCaretHotKeyAction: UInt32, CaseIterable {
...
struct VoxCaretHotKey: Equatable {
...
///
/// The persisted shortcut shape is kept separate from Carbon registration so the part
/// worth writing once is the fallback rule, not the pair of key names.
struct VoxCaretHotKeyBinding {
    let enabledKey: String
    let hotKeyKey: String
    let defaultEnabled: Bool
    let defaultHotKey: VoxCaretHotKey
    ...
    var hotKey: VoxCaretHotKey {
        guard let raw = UserDefaults.standard.string(forKey: hotKeyKey),
              let hotKey = VoxCaretHotKey(storageValue: raw),
              hotKey.isValid else {
...
extension VoxCaretHotKeyAction {
    /// The preference this action reads, or nil when nothing about it is bindable.
    ///
    /// `cancelVoice` has none on purpose: it is ⎋, VoxCaret owns it, and it only exists
    /// while dictation is running — see `VoxCaretGlobalHotKeyManager.registerTransient`.
    var binding: VoxCaretHotKeyBinding? {
        switch self {
        case .voiceInput: return VoxCaretInputPreferences.hotKeyBinding
        case .cancelVoice: return nil
        }
    }
}

/// Holds VoxCaret's system-wide shortcuts.
///
...
@MainActor
final class VoxCaretGlobalHotKeyManager: ObservableObject {
    static let shared = VoxCaretGlobalHotKeyManager()
    ...
    enum Registration: Equatable {
        case off
        case active(VoxCaretHotKey)
        case unavailable(VoxCaretHotKey)
    }

    @Published private(set) var registrations: [VoxCaretHotKeyAction: Registration] = [:]
    ...
    var onPressed: ((VoxCaretHotKeyAction) -> Void)?
    var onReleased: ((VoxCaretHotKeyAction) -> Void)?

    private var refs: [VoxCaretHotKeyAction: EventHotKeyRef] = [:]
    private var eventHandler: EventHandlerRef?
    private var defaultsObserver: NSObjectProtocol?

    func registration(for action: VoxCaretHotKeyAction) -> Registration {
    ...
    func start() {
        ...
        defaultsObserver = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in VoxCaretGlobalHotKeyManager.shared.apply() }
        }
    }
    ...
    func stop() {
        ...
        for action in VoxCaretHotKeyAction.allCases {
            unregister(action)
        }
        registrations = [:]
        if let eventHandler {
    ...
    /// Takes a shortcut VoxCaret owns outright for as long as something is running,
    /// bypassing both the preference table and `VoxCaretHotKey.isValid`.
    ///
    ...
    func registerTransient(_ action: VoxCaretHotKeyAction, hotKey: VoxCaretHotKey) {
        register(action, hotKey, requireModifiers: false)
    }

    func unregisterTransient(_ action: VoxCaretHotKeyAction) {
        unregister(action)
        registrations[action] = .off
    }

    private func register(_ action: VoxCaretHotKeyAction, _ hotKey: VoxCaretHotKey, requireModifiers: Bool = true) {
        unregister(action)
    ...
    private func unregister(_ action: VoxCaretHotKeyAction) {
    ...
    private func installEventHandler() {
        ...
        if status != noErr {
            VoxCaretLog.failure("global_hotkey_handler_install_failed", error: "OSStatus \(status)")
        }
    }

    fileprivate func handle(action: VoxCaretHotKeyAction, isPressed: Bool) {
        if isPressed {
            onPressed?(action)
        } else {
            onReleased?(action)
...
private func atmGlobalHotKeyEventHandler(
    ...
    guard status == noErr,
          ...
          let action = VoxCaretHotKeyAction(rawValue: identifier.id) else {
    ...
    Task { @MainActor in
        VoxCaretGlobalHotKeyManager.shared.handle(action: action, isPressed: isPressed)
    }
    return noErr
}
