...
enum VoxCaretSenseVoiceTranscriberError: LocalizedError {
    case modelMissing
...
@MainActor
final class VoxCaretSenseVoiceTranscriber: VoxCaretSpeechTranscribing {
    ...
    private var accumulator = VoxCaretSenseVoiceAudioAccumulator()

    func requestAuthorization() async throws {
        try await VoxCaretMicrophoneAuthorization.request()
        guard VoxCaretSenseVoiceModelManager.shared.isModelReady else {
            throw VoxCaretSenseVoiceTranscriberError.modelMissing
        }
    }
    ...
    func prepare() async {
        guard let files = VoxCaretSenseVoiceModelManager.shared.modelFiles else { return }
        try? await VoxCaretSenseVoiceEngine.shared.prepare(
            files: files,
            language: VoxCaretInputPreferences.language.senseVoiceCode
        )
    }
    ...
    func start(
        ...
        cancel()
        guard VoxCaretSenseVoiceModelManager.shared.modelFiles != nil else {
            throw VoxCaretSenseVoiceTranscriberError.modelMissing
        }
        ...
        let accumulator = VoxCaretSenseVoiceAudioAccumulator()
    ...
    func finish() async throws -> String {
        ...
        guard audio.samples.count >= max(1, Int(audio.sampleRate / 10)) else {
            throw VoxCaretSenseVoiceTranscriberError.noAudio
        }
        guard let files = VoxCaretSenseVoiceModelManager.shared.modelFiles else {
            throw VoxCaretSenseVoiceTranscriberError.modelMissing
        }
        return try await VoxCaretSenseVoiceEngine.shared.transcribe(
...
private final class VoxCaretSenseVoiceAudioAccumulator: @unchecked Sendable {
...
///
/// It unloads itself after five idle minutes. VoxCaret is a menu bar app that runs all
...
actor VoxCaretSenseVoiceEngine {
    static let shared = VoxCaretSenseVoiceEngine()
    ...
    func prepare(files: VoxCaretSenseVoiceModelFiles, language: String) throws {
        ...
        guard let created else { throw VoxCaretSenseVoiceTranscriberError.modelLoadFailed }
        recognizer = created
