...
enum VoxCaretTextCleanup {
    ...
    static func parseReplacements(_ raw: String) -> [VoxCaretReplacement] {
        raw.split(whereSeparator: \.isNewline).compactMap { line in
            ...
            return VoxCaretReplacement(source: source, target: target)
        }
    }

    static func applyReplacements(_ value: String, _ replacements: [VoxCaretReplacement]) -> String {
    ...
    static func process(
        _ transcript: String,
        replacements: [VoxCaretReplacement],
    ...
    static func process(_ transcript: String) -> String {
        process(
            ...
            removeTrailingPeriod: VoxCaretInputPreferences.removeTrailingPeriod
        )
    }
}
