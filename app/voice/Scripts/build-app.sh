...
NATIVE_APP="$NATIVE_STAGING/ATM Voice.app"
mkdir -p "$NATIVE_APP/Contents/MacOS" "$NATIVE_APP/Contents/Resources"
cp "$NATIVE_ROOT/.build/$NATIVE_CONFIGURATION/ATMVoice" "$NATIVE_APP/Contents/MacOS/ATMVoice"
cp "$NATIVE_ROOT/Resources/Info.plist" "$NATIVE_APP/Contents/Info.plist"
chmod +x "$NATIVE_APP/Contents/MacOS/ATMVoice"
"$NATIVE_ROOT/../../scripts/codesign-local.sh" "$NATIVE_APP"
codesign --verify --strict "$NATIVE_APP"
rm -rf "$NATIVE_ROOT/dist/ATM Voice.app"
mv "$NATIVE_APP" "$NATIVE_ROOT/dist/ATM Voice.app"
print "$NATIVE_ROOT/dist/ATM Voice.app"
