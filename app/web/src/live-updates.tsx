...
const routes: Record<string, string[]> = {
  tasks: ['todos', 'presence', 'jobs'],
  collection: ['collection', 'jobs'],
  agents: ['sessions', 'presence', 'jobs'],
  knowledge: ['knowledge', 'memory'],
  usage: ['usage', 'jobs'],
  'ai-day': ['day', 'jobs'],
  settings: ['settings', 'jobs', 'presence'],
}
const queryDomains: Record<string, string[]> = {
  todos: ['todos', 'todo', 'doc', 'dependencies'],
  collection: ['collection'],
  sessions: ['activity-status', 'activity-sessions', 'activity-search', 'activity-session'],
  usage: ['activity-usage', 'activity-quota'],
  knowledge: ['knowledge'],
  memory: ['knowledge'],
  day: ['day.snapshot', 'day.show', 'day.ledger'],
  settings: ['settings.get'],
  jobs: ['runtime-jobs', 'runtime-job'],
  presence: ['activity-status', 'presence'],
}
...
export function useLiveUpdates(boot: Bootstrap | undefined) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      source.onerror = () => {
        ...
        void fetch('/api/v1/bootstrap', { credentials: 'same-origin', cache: 'no-store' })
          ...
          .catch(() => {
            /* EventSource retries temporary disconnects. */
          })
          .finally(() => {
            probing = false
          })
      ...
      source.addEventListener('resource.changed', (event) => {
        try {
          ...
          if (Array.isArray(data.domains))
            invalidate(data.domains.filter((value): value is string => typeof value === 'string'))
        } catch {
          /* Polling and refocus remain available if an event is malformed. */
        }
      })
    ...
    const visibility = () => {
      connect()
      if (document.visibilityState === 'visible') invalidate(domains)
    }
    connect()
