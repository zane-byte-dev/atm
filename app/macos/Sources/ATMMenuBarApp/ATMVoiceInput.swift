...
@MainActor
protocol ATMSpeechTranscribing: AnyObject {
...
extension ATMSpeechTranscribing {
    var inputLevel: Float { 0 }
    func prepare() async {}
}

enum ATMSpeechTranscriberError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .requiresAppBundle:
            return "语音输入需要以 .app 运行。`swift run` 是裸可执行文件，系统弹不出麦克风授权框。改用 Scripts/run-dev-app.sh。"
        case .authorizationTimedOut:
            return "等授权等超时了。如果没看到系统弹窗，多半是这个进程不是正规 .app；否则到「系统设置 → 隐私与安全性」里手动允许。"
        case .microphoneDenied:
            return "没有麦克风权限。到「系统设置 → 隐私与安全性 → 麦克风」里允许 ATM 使用麦克风。"
        case .speechRecognitionDenied:
            return "没有语音识别权限。到「系统设置 → 隐私与安全性 → 语音识别」里允许 ATM。"
        case .recognizerUnavailable:
            return "系统语音识别当前不可用。"
        case .onDeviceRecognitionUnavailable:
...
enum ATMVoiceRecognitionEngine: String, CaseIterable, Identifiable {
    case senseVoice
    case appleSpeech
...
enum ATMVoiceInputLanguage: String, CaseIterable, Identifiable {
    case auto
    case chinese = "zh-CN"
    case english = "en-US"
    case japanese = "ja-JP"
...
struct ATMVoiceReplacement: Equatable {
    let source: String
    let target: String
}

/// Where dictation's settings live. Same shape as `ATMGlobalHotKeyPreferences` and
/// `ATMAgentNotchPreferences`: raw UserDefaults keys the settings screen binds to
/// with `@AppStorage`, plus resolved accessors for everyone else.
enum ATMVoiceInputPreferences {
    static let hotKeyEnabledKey = "ATMVoiceInputHotKeyEnabled"
    static let hotKeyKey = "ATMVoiceInputHotKey"
    static let engineKey = "ATMVoiceInputEngine"
    static let languageKey = "ATMVoiceInputLanguage"
    static let onDeviceOnlyKey = "ATMVoiceInputOnDeviceOnly"
    static let removeTrailingPeriodKey = "ATMVoiceInputRemoveTrailingPeriod"
    static let dictionaryKey = "ATMVoiceInputDictionary"

    static let defaultEnabled = true
    static let defaultEngine = ATMVoiceRecognitionEngine.senseVoice
    static let defaultLanguage = ATMVoiceInputLanguage.auto
    ...
    static let defaultHotKey = ATMHotKey(keyCode: UInt16(kVK_Space), modifiers: [.option])

    static let hotKeyBinding = ATMHotKeyBinding(
    ...
    /// ⎋ while recording. Not stored anywhere — ATM owns it and only for the few
    /// seconds a recording lasts.
    static let cancelHotKey = ATMHotKey(keyCode: UInt16(kVK_Escape), modifiers: [])

    /// Longest single recording. A key that never reports coming back up would
    /// otherwise hold the microphone open until the app quits.
    static let maximumRecordingDuration: Duration = .seconds(60)

    static var isHotKeyEnabled: Bool { hotKeyBinding.isEnabled }
    static var hotKey: ATMHotKey { hotKeyBinding.hotKey }
    ...
    static var engine: ATMVoiceRecognitionEngine {
        guard let raw = UserDefaults.standard.string(forKey: engineKey),
              let engine = ATMVoiceRecognitionEngine(rawValue: raw) else {
            return defaultEngine
        }
        return engine
    }

    static var language: ATMVoiceInputLanguage {
        guard let raw = UserDefaults.standard.string(forKey: languageKey),
              let language = ATMVoiceInputLanguage(rawValue: raw) else {
    ...
    static var replacements: [ATMVoiceReplacement] {
        ATMVoiceTextCleanup.parseReplacements(UserDefaults.standard.string(forKey: dictionaryKey) ?? "")
    }
...
struct ATMVoicePermissionSnapshot: Equatable {
    let microphone: ATMVoicePermissions.Status
    let speechRecognition: ATMVoicePermissions.Status
    let accessibility: ATMVoicePermissions.Status

    static func current() -> ATMVoicePermissionSnapshot {
        ATMVoicePermissionSnapshot(
            microphone: ATMVoicePermissions.microphone,
            speechRecognition: ATMVoicePermissions.speechRecognition,
            accessibility: ATMVoicePermissions.accessibility
        )
    }
}
