...
@MainActor
final class ATMVoiceTranscriberRouter: ATMSpeechTranscribing {
    private let apple: ATMSpeechTranscribing
    private let senseVoice: ATMSpeechTranscribing
    private var active: ATMSpeechTranscribing?

    init(
        apple: ATMSpeechTranscribing? = nil,
        senseVoice: ATMSpeechTranscribing? = nil
    ) {
        self.apple = apple ?? ATMAppleSpeechTranscriber()
        self.senseVoice = senseVoice ?? ATMSenseVoiceTranscriber()
    }
    ...
    private func resolved() -> ATMSpeechTranscribing {
        guard ATMVoiceInputPreferences.engine == .senseVoice,
              ATMSenseVoiceModelManager.shared.isModelReady else {
            return apple
        }
        return senseVoice
    }
